...
>
> 그 외에도 AWS와 같은 클라우드 환경을 사용할때도 이미지 단위의 배포가 가능하며,
...
2. `$ echo hello, world`
이전의 `hello world`는 호스트 환경에서 실행한 Ubuntu 명령어 였지만 지금은 직접 Ubuntu shell로 들어와
...
> **No**
실제로 굉장히 비슷하지만 Docker와 VM 과는 여러가지 다른 점이 존재합니다.
그 중 하나가 하드웨어 가상화입니다.
VirtualBox나 VM은 하드웨어 가상화가 이루어집니다.
...
도커는 특이하게 계층화 된 파일 시스템을 사용하는데 이미지 위의 이미지를 엎는 방식을 사용하고 있다.
...
## 5. Docker Demo 02
1. 실제로 작동하는 애플리케이션 (mysql)
`$ docker run -p 3000:3000 --name mysql -d mysql`
도커에서 실행되는 포트와 호스트에서 사용되는 포트를 연결해주고 mysql 이름을 붙여 mysql을 실행해보겠습니다.
2. 컨테이너 확인
`$ docker ps`
컨테이너가 하나 생성된 것을 확인하셨나요?
저희는 명령어 한 줄을 통해 mysql을 실행했습니다! (Docker의 멋짐)

(프로세스를 하나 띄운 것 // 물론 리눅스 환경이였다면 정말 프로세스를 하나 띄운것이 겠지만
맥이나 윈도우에서는 리눅스 커널의 기능을 직접적으로 사용할 수 없기 때문에 가상화 위에서 돌린다고 생각)

(컨테이너가 필요한 이유)
컴퓨터의 환경이 모두 다르다

## 6. Docker Container Command
1. 컨테이너 확인
`$ docker ps`
...
3. git 설치
`$ apt-get install git`
> **기본 우분투 이미지로 실행한 컨테이너에 git이 설치 된 상태**
...
> 보통 Commit을 이용해 이미지를 만드는 경우는 거의 없고 Dockerfile을 이용해 이미지를 만들게 됩니다.

> 이미지가 생성되었는지 확인해보겠습니다.
`$ docker images | grep ubuntu:git02`
> ubuntu:git02 라는 이미지가 생성된 것을 확인 할 수 있습니다.
...
3. RUN
> RUN 명령어는 `RUN <Command>`로 실행할 수 있으며, 컨테이너를 실행한 후 실행하는 명령어를 작성합니다.
> 보통 bash를 통해 사용하지 않고, Dockerfile을 이용하여 실행합니다.
...
> -t는 이미지에 이름을 부여하고 tag는 latest 또는 01과 같이 자유롭게 입력하시면 됩니다.
> -t 옵션을 사용하는 이유는 이후에 이미지를 빠르게 검색하기 위함입니다.
